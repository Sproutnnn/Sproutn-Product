-- Add prototyping-related fields to projects table
...
COMMENT ON COLUMN projects.prototype_status IS 'Status within the prototyping phase: producing, shipping, delivered, feedback';
COMMENT ON COLUMN projects.tracking_number IS 'Shipping tracking number for prototype delivery';
COMMENT ON COLUMN projects.estimated_delivery IS 'Estimated delivery date for prototype';
COMMENT ON COLUMN projects.admin_notes IS 'Admin notes about prototype progress';
...
COMMENT ON COLUMN projects.customer_feedback IS 'Customer feedback on the prototype';
